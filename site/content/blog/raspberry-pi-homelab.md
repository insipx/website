...
> this is probably “part 1” of a series. I will cover my reasoning for why I
> made certain choices for this build, as well as how to set up the initial
> Raspberry Pi NixOS nodes, including k3s, colmena, kubenix, ZFS, and Longhorn.
> All of it should be possible through the templates and modules defined in
> `nixos-rpi-lab.` although I may create further fixes/additions as time goes
> on.
...
wanted something I could set up and run what I need with minimal intervention.
After a month, I would begin to forget the software I installed, or the little
...
This time would be different. I’m a huge NixOS fan; I used it on anything I can.
Nix allowed me to set up my machines declaratively, and I never felt like there
was “random” software floating around I forgot about. I began to think about how
I could use NixOS to manage my self-hosted devices.
...
fly.io. It needed to offer a similar developer experience -- something where I
could throw up my nix-built Docker Images and have a working host on my internal
LAN. Finally, I wanted updates to be easier. If I’m running 3+ devices, I am
_not_ ssh’ing each time to run a script manually. It had to be updatable.

## Why NixOS

Determinism and declarative configuration of Nix made it attractive to handle
the configuration for multiple nodes. Nix opens the door for caching, or even
...
[Colmena](https://colmena.cli.rs/unstable/). I came across it previously on
GitHub, and it was also a Rust tool (which I always appreciate).

During my work on this project, Raspberry Pis struggled compiling larger pieces
of software. Software for `aarch64-linux` did not always appear in the binary
cache for `nixos-raspbberypi`. Setting up an on-demand beefy build server on AWS
to speed up deploys is a great cure for my impatience at the cost of a few
dollars. NixOS makes this workflow easy.

## Why K3s

I decided K3S is the way. K3S is a lighter distribution of K8S that I run with
the embedded `etcd`. Part of this project was meant for me to learn Kubernetes,
and K3S is what would enable me to run the Docker Images with the workflow I
expected.

### Kubenix

The one thing unattractive with regards to Kubernetes, however, was the YAML
manifest format. Spending way too much time configuring GitHub Actions scared
...
to deploy existed. Luckily, I was able to find the
...
so deploys more accurately reflect what exists on the cluster.

I found this problem solved by Kubenix. With Kubenix I could have:

- one language for everything
- resources type-checked against the API schema at eval time
- hash-pinned charts
- prune on apply
...
For the initial version, I ended up with this to host three Kubernetes Control
nodes. That would give me redundancy. K3S control plane nodes act as worker
nodes. It’s possible to run k3s on a single node, if one desires. I did _not_
have a 3d printer at this point, and went with the GeeekPi rack they sell. The
RPi5 piece includes an NVMe SSD adapter.
...
- [1x PoE switch to power the Pis. I went with GigaPlus 2.5G PoE Switch](www.servethehome.com/the-ultimatze-cheap-2-5gbe-switch-mega-round-up-buyers-guide-qnap-netgear-hasivo-mokerlink-trendnet-zyxel-tp-link/)
- 3x PoE Hat, I went with the one from
...
- probably a couple of SD cards for the initial NixOS install
...
```text
.
├── base/                # NixOS Config used by everything. Modify the user config here.
├── deployments/         # Stuff that's deployed to the cluster
│   └── kubenix/         # Kubernetes resources defined with Kubenix
├── hive/                # Colmena NixOS Deployment configurations for each node
...
```text
.
├── .sops.yaml # sops public keys/config values
├── flake.nix  # devshell with `sops` and `rage` and a `keyscan` package
├── keyscan.sh # small bash script to simplify scanning and converting SSH public keys to `age` public key format
...
In the past, I’ve found the IP by checking myrouter'ss DHCP leases for the new
one. You should be able to SSH, which is important for the nixos-anywhere
command.
...
initial disk setup. The setup is almost complete.
...
Towards the bottom is defined an ext4 overlay is defined. The homelab module
allows configuring the overlay size (on _initial_ install). Longhorn does not
work with ZFS; we need to create an overlay to make use of it. This can be
skipped if the install is a pure SD-card install or if you choose a filesystem
other than ZFS which Longhorn supports natively.
...
This will install and reboot. At that point, you can remove the SD card and
reboot again to the NixOS install from the SD card.

## The HomeLab NixOS Module

Much of the per-node configuration exists in the homelab module I created for
configuring nodes.
[The documentation](https://github.com/insipx/nixos-rpi-lab/#nixos-homelab-module-configuraiton)
...
rpiHomeLab = {
  ...
  k3s = {
    ...
    longhorn = true;
    longhornDiskSize = "800G"; # Adjust for available storage.
  };
};
```

`longhornDiskSize` should be changed depending on the nodes' available storage.
Leave some room for the OS; in this case, `800G` is giving 800 Gigabytes for
Longhorn and leaving ~200 Gigabytes for the OS. Longhorn itself can be disabled
with `longhorn = false`. This will avoid creating the ext4 ZFS overlay.
...
At this point, colmena should be able to build/apply to nodes. Ensure that
`k3s.` is disabled, however, since k3s requires extra steps to bootstrap the
secret correctly.
...
have access to the server token. If setting up additional worker nodes, those
worker nodes should have access to a separate worker token, which restricts what
a worker node may do with the cluster.
...
#### Setup sops secrets repository

At the start, we initialized a secrets repository but didn’t do anything with
it. In `.sops.yaml` of the secrets repository, add the public keys to encrypt
...
Once you have the keys, the secrets need to be created.
...
For this guide, let’s use `eujcjs.7bwfdy5vdh93xy6p` as the secret (do not reuse
this for obvious reasons). This is the value that will go into the
`secrets/homelab.yaml` file.

Before encryption, `homelab.yaml` will look like:

```yaml
k3s_token: eujcjs.7bwfdy5vdh93xy6p
```
...
node1 = _: {
   ...
   deployment = {
     targetHost = "node1.lab.lan";
     targetUser = "user"; # your user on node1
     tags = [
       "homelab"
       "control"
     ];
   ...
   rpiHomeLab = {
     networking = {
       hostId = "00000000"; # ensure this is a unique id
       hostName = "node1"; 
   ...
   rpiHomeLab.k3s.longhorn = false; # longhorn is not required at this stage
   lab-secrets.settings.k3s = true; # this must be enabled to access the k3s_token
   services.k3s.extraFlags = [ 
   # This registers extra hostnames with the k3s internal Certificate Authority.
   # Enables referring to this node with the names listed
...
The leader address is in `base/default.nix` towards the bottom. There’s only one
leader, and it should be the same for all nodes. Set the address to the node
that we just set up as the leader.
...
`~/.kube/config` to use `kubectl` with the cluster.
...
```
~ at ☸️  default took 37s 713ms
 [I] ➜ kubectl get nodes
NAME       STATUS     ROLES                       AGE    VERSION
amalthea   Ready      <none>                      250d   v1.35.6+k3s1
callisto   Ready      <none>                      308d   v1.35.6+k3s1
elara      Ready      <none>                      97d    v1.35.6+k3s1
europa     Ready      control-plane,etcd,master   308d   v1.35.6+k3s1
ganymede   Ready      control-plane,etcd,master   308d   v1.35.6+k3s1
io         Ready      control-plane,etcd,master   308d   v1.35.6+k3s1
lysithea   Ready      <none>                      244d   v1.35.6+k3s1
sinope     NotReady   <none>                      308d   v1.34.2+k3s1
```
...
Homelab NixOS module configuration to allow for Metal LB.
...
On my setup, I created a VLAN on `10.10.69.0/22`. This allows me to give Metal
LB multiple address pools in `69.0/24`. The DHCP server hands out leases for
physical machines on a limited range in the `10.10.69.0/24` space. That gives me
